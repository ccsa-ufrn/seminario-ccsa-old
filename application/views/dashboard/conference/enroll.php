<div class="row">
    <div class="col-md-12">
        <h1 class="page-header">Inscrever-se em Conferências</h1>
        ...
        <h4 class="page-header">Minhas Conferências</h4>
        
        <table class="table table-hover">

        <?php foreach($user->sharedConferenceList as $mc): ?>
            ...
            <tr class="active" style="text-align:center;" >
                ...
                <td style="width:20%">
                    <?php echo date("d/m/Y", strtotime($mc->conferencedayshift->date));  ?>
                </td>
                <td>
                    <a style="cursor:pointer;" data-target="#modalEnrollConferenceDetails" data-toggle="modal" data-data="<?php echo  $mc->id;?>">Detalhes</a>
                </td>
                <td>
                    <?php if($date_limit['open']): ?>
                    <a style="cursor:pointer;"  class="conference-registration-button-unroll" data-data="<?php echo $mc->id; ?>" >Desfazer Inscrição</a>
                    <?php echo form_open(base_url('dashboard/conference/unroll'), array('id' => 'formUnrollConference-'.$mc->id,'novalidate' => '','class' => 'waiting','style' => 'display:inline-block;')); ?>
        ...
        <?php if(!count($user->sharedConferenceList)): ?>
            <tr class="danger">
                <?php if($date_limit['open']): ?>
                <td>Você ainda não se inscreveu em nenhuma conferência.</td>
                <?php else: ?>
                <td>Você não se inscreveu em nenhuma conferência do seminário.</td>
                <?php endif; ?>
            </tr>
        <?php endif; ?>  
        ...
        <h4 class="page-header">Inscrever-se em Conferências</h4>
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-12">
                                        ...
                                        <?php if(count($e['conferences'])==0): ?>
                                            <tr class="danger">
                                                <td>Ainda não há conferências cadastradas para este dia/turno.</td>
                                            </tr>
                                        <?php else: ?>
                                            ...
                                            <?php foreach($e['conferences'] as $m): ?>
                                                ...
                                                <?php 
                                                    ...
                                                    if($m->vacanciesfilled==$m->vacancies)
                                                        $status = 'novacancies';
                                                        
                                                    if(R::count('conferenceUser','user_id=? AND conference_id=?',array($user->id,$m->id)))
                                                        $status = 'enrolled';
                                                ...
                                                style="text-align:center;">
                                                    ...
                                                    <td style="width:20%">
                                                        <?php echo $m->vacancies - $m->vacanciesfilled ?> Vagas
                                                    </td>
                                                    <td>
                                                        <a style="cursor:pointer;" data-target="#modalEnrollConferenceDetails" data-toggle="modal" data-data="<?php echo $m->id; ?>">Detalhes</a>
                                                    </td>
                                                    <?php if($status=='vacancies'): ?> 
                                                        <td>
                                                            <a style="cursor:pointer;" class="conference-registration-button" data-data="<?php echo $m->id; ?>" >Inscrever-se</a>
                                                        </td>
                                                        <?php echo form_open(base_url('dashboard/conference/enrolla'), array('id' => 'formEnrollConference-'.$m->id,'novalidate' => '','class' => 'waiting','style' => 'display:inline-block;')); ?>
    ...
    <?php else: ?>
        ...
        <?php elseif( $user->paid!='accepted' && $user->paid!='free' ): ?>
            Você precisa realizar <a href="<?php echo base_url('dashboard/user/payment'); ?>">o pagamento</a> para se inscrever em uma conferência.
...
<!-- Modal -->
    <div class="modal fade" id="modalEnrollConferenceDetails" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <input id="cmrd" style="display:none;" value="<?php echo base_url('dashboard/conference/retrieveenrolldetails'); ?>" />