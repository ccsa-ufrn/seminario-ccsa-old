<div class="row">
    <div class="col-md-12">
        <h1 class="page-header">Inscrever-se em Oficinas</h1>
        ...
        <h4 class="page-header">Minhas Oficinas</h4>
        
        <table class="table table-hover">

        <?php foreach($user->sharedWorkshopList as $mc): ?>
            ...
            <tr class="active" style="text-align:center;" >
                ...
                <td style="width:20%;vertical-align: middle;">
                    <?php $i = 0; ?>
                    <?php foreach($mc->sharedWorkshopdayshiftList as $mds): ?>
                        <?php if($i!=0) echo ','; ?>
                        <?php echo date("d/m/y", strtotime($mds->date))." (".$mds->shift.") "; ++$i; ?>
                    <?php endforeach; ?>
                </td>
                <td style="vertical-align: middle;">
                    <a style="cursor:pointer;" data-target="#modalEnrollWorkshopDetails" data-toggle="modal" data-data="<?php echo  $mc->id;?>">Detalhes</a>
                </td>
                <td style="vertical-align: middle;">
                    <?php if($date_limit['open']): ?>
                    <a style="cursor:pointer;"  class="workshop-registration-button-unroll" data-data="<?php echo $mc->id; ?>" >Desfazer Inscrição</a>
                    <?php echo form_open(base_url('dashboard/workshop/unroll'), array('id' => 'formUnrollWorkshop-'.$mc->id,'novalidate' => '','class' => 'waiting','style' => 'display:inline-block;')); ?>
        ...
        <?php if(!count($user->sharedWorkshopList)): ?>
            <tr class="danger">
                <td>Você ainda não se inscreveu em nenhum oficina.</td>
            </tr>
        <?php endif; ?>  
            
        </table>
        
        <h4 class="page-header">Inscrever-se em Oficinas</h4>
                        ...
                        <?php foreach ($ws as $item): ?>
                            ...
                            <table class="table table-hover">
                                ...
                                <?php 
                                    ...
                                    if($item->vacanciesfilled==$item->vacancies)
                                        $status = 'novacancies';
                                        
                                    if(R::count('userWorkshop','user_id=? AND workshop_id=?',array($user->id,$item->id)))
                                        $status = 'enrolled';
                                ...
                                style="text-align:center;">
                                    ...
                                    <td style="vertical-align: middle;">
                                        <?php $i = 0; ?>
                                        <?php foreach($item->sharedWorkshopdayshiftList as $mds): ?>
                                            <?php if($i!=0) echo ','; ?>
                                            <?php echo date("d/m/y", strtotime($mds->date))." (".$mds->shift.") "; ++$i; ?>
                                        <?php endforeach; ?>
                                    </td>
                                    <td style="vertical-align: middle;padding:0px 20px;">
                                        <?php echo $item->vacancies - $item->vacanciesfilled; ?> Vagas
                                    </td>
                                    <td style="vertical-align: middle;padding:0px 20px;">
                                        <a style="cursor:pointer;" data-target="#modalEnrollWorkshopDetails" data-toggle="modal" data-data="<?php echo $item->id; ?>">Detalhes</a>
                                    </td>
                                    <?php if($status=='vacancies'): ?> 
                                        <td style="vertical-align: middle;padding:0px 20px; min-width:170px;">
                                            <a style="cursor:pointer;" class="workshop-registration-button" data-data="<?php echo $item->id; ?>" >Inscrever-se</a>
                                        </td>
                                        <?php echo form_open(base_url('dashboard/workshop/enrolla'), array('id' => 'formEnrollWorkshop-'.$item->id,'novalidate' => '','class' => 'waiting','style' => 'display:inline-block;')); ?>
    ...
    <?php else: ?>
        ...
        <?php elseif( $user->paid!='accepted' && $user->paid!='free' ): ?>
            Você precisa realizar <a href="<?php echo base_url('dashboard/user/payment'); ?>">o pagamento</a> para se inscrever em uma oficina.
...
<!-- Modal -->
    <div class="modal fade" id="modalEnrollWorkshopDetails" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <input id="mmmssrd" style="display:none;" value="<?php echo base_url('dashboard/workshop/retrieveenrolldetails'); ?>" />